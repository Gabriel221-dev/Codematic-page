import "./Home.css";
import { AiOutlineCloudSync } from "react-icons/ai";
import { HiOutlineViewfinderCircle } from "react-icons/hi2";
import { FaDatabase } from "react-icons/fa";
import { SiMaterialdesignicons } from "react-icons/si";
import { TbSunset2 } from "react-icons/tb";
import { IoMdArrowForward } from "react-icons/io";

function home() {
  return (
    <div className="Home">
      <section className="Home-1">
        <div>
          <div>
            <a href="" alt="" className="academy-1">
              <mark className="mark">Codematic Academy</mark>Join the Academy
              <IoMdArrowForward />
              <em></em>
            </a>
          </div>
          <div className="heading-sub-1">
            <div className="heading-1">
              Empowering Africa&apos;s best talents with modern technology
              skills and access to global opportunities{" "}
            </div>
            <div className="sub-heading-1">
              — We are training 1,000,000 African youths in Cloud Technology,
              AI, and Open Source by 2034, unlocking global opportunities and
              fostering innovation.
            </div>
          </div>
          <div className="action">
            <button className="learn-heading">
              <i>
                <TbSunset2 />
              </i>
              Learn More
            </button>
            <button className="apply-heading">
              Apply Now
              <i>
                <IoMdArrowForward />
              </i>
            </button>
          </div>
          <div className="frame">
            <img src="/Images/Frame 2353.png" alt="" />
          </div>
        </div>
      </section>
      <section className="Home-2">
        <div className="leg">
          <div className="heading-sub-2">
            <div className="sub-heading-2">About us</div>
            <div className="heading-2">Why we started Codematic Academy</div>
            <article className="text-2">
              <p>
                Codematic Group provides software development, technology
                consulting and cloud managed services for organisations in
                Africa and the rest of the world. Although headquartered in the
                United Kingdom, we maintain a satellite office in Lagos,
                Nigeria, where many of its employees are based.
              </p>
              <p>
                Our operations in Nigeria revealed a large skill gap between
                African youths and their counterparts in developed markets in
                Europe and the US. This severely limits economic growth and
                access to job opportunities, which are already scarce in Africa.
              </p>
              <p>
                Therefore, we created the Codematic Academy as a non-profit arm
                of Codematic Group. Through the Academy, we resolved to bridge
                the skill and opportunity gap, while preparing Africa’s youth
                for the future of work
              </p>
            </article>
            <button className="apply-now-2">Apply Now</button>
          </div>
          <div className="img">
            <img src="/Images/Image.png" alt="" />
          </div>
        </div>
      </section>
      <section className="Home-3">
        <container>
          <content className="body-3">
            <article className="heading-sub-3">
              <h3 className="sub-3">Our Goals</h3>
              <div className="heading-3">We do things differently...</div>
              <div className="text-3">
                To rigorously train and create employment opportunities for
                1,000,000 brilliant youths in Africa (starting with Nigeria), in
                Cloud Technology, Artificial Intelligence and Open Source
                Technology By 2034
              </div>
            </article>
          </content>
          <div className="img3">
            <img src="/Images/Metrics.png" alt="" />
          </div>
        </container>
      </section>
      <section className="Home-4">
        <container>
          <content>
            <div className="H-4">
              <article className="header-sub-4">
                <div className="sub-header-4">Our Technovation Focus</div>
                <div className="header-4">Our Areas of Focus</div>
                <div className="text-4">
                  We are dedicated to empowering Africa&apos;s youth by focusing
                  on four key areas:
                </div>
              </article>
            </div>
          </content>
          <content className="H-4-1">
            <div className="H-4-2">
              <div className="row">
                <article className="part-1">
                  <div className="icon cloud">
                    <AiOutlineCloudSync />
                  </div>
                  <h2 className="header-text-4">
                    Google Cloud & Open Source Development
                  </h2>
                  <h1 className="text-4">
                    Harness the power of open source technologies on the
                    scalable and secure infrastructure of Google Cloud. Unlock
                    new possibilities with robust tools and seamless
                    integrations.
                  </h1>
                </article>
                <article className="part-2">
                  <div className="icon circle">
                    <HiOutlineViewfinderCircle />
                  </div>
                  <h2>Artificial intelligence on Google Cloud</h2>
                  <h1 className="text-4">
                    Leverage Google&apos;s cutting-edge artificial intelligence
                    technologies to drive innovation and efficiency. With
                    powerful AI and machine learning tools, automate processes,
                    and deliver personalized experiences at scale.
                  </h1>
                </article>
              </div>
              <div className="row">
                <article className="part-3">
                  <div className="icon data">
                    <FaDatabase />
                  </div>
                  <h2>Data Analytics/Business Intelligence on Google Cloud</h2>
                  <h1 className="text-4">
                    Transform raw data into actionable insights with scalable,
                    secure, and intuitive solutions. From real-time analytics to
                    predictive modeling, drive smarter decisions, optimize
                    operations, and uncover new opportunities.
                  </h1>
                </article>
                <article className="part-4">
                  <div className="icon">
                    <SiMaterialdesignicons />
                  </div>
                  <h2>UI/UX Design</h2>
                  <h1 className="text-4">
                    Create intuitive and engaging interfaces that captivate your
                    audience and drive user satisfaction. Through innovative
                    UI/UX design, transform complex ideas into seamless,
                    user-friendly experiences.
                  </h1>
                </article>
              </div>
            </div>
          </content>
        </container>
      </section>
      <section className="Home-5">
        <div className="Home-5-1">
          <div className="img">
            <img src="/Images/ImageYinka.png" alt="" />
          </div>
          <div className="heading-sub-5">
            <article>
              <div className="Founder">
                <h1>Meet Our Founder</h1>
                <div>
                  <p>
                    Yinka is a senior business and technology leader, formerly
                    at Google and Goldman Sachs.
                  </p>
                  <p>
                    Yinka co-founded the Codematic Group to bridge the
                    technology gap in Africa, and transform how organisations
                    operate. He has an unwavering belief in Africa’s potential
                    and envisions its youth as the most sought-after tech
                    talents globally.
                  </p>
                </div>
              </div>
            </article>
          </div>
        </div>
      </section>
      <section className="Home-6">
        <container>
          <div className="big-6">
            <div className="sub-Head-6">Testimonials</div>
            <div className="Head-6">Meet some of our recent talents</div>
            <div className="Text-6">
              Hear from some of our amazing interns who are fostering
              innovations.
            </div>
          </div>
          <content>
            <div className="Team-6-1">
              <div className="Team-6">
                <article className="Member-61">
                  <img src="/Images/ImageIfe.png" alt="" />
                  <h2>Ife</h2>
                  <p>Data Analytics/ Data Engineering</p>
                </article>
                <article className="Member-62">
                  <img src="/Images/ImageAYO.png" alt="" />
                  <h2>Ayomide</h2>
                  <p>Google Cloud and Open Source Software Development</p>
                </article>
                <article className="Member-63">
                  <img
                    src="/Images/WhatsApp Image 2024-06-09 at 09.54.19 1Uche.png"
                    alt=""
                  />
                  <h2>Uche</h2>
                  <p>UI/UX Design, BI/Analytics Dashboards</p>
                </article>
              </div>
            </div>
          </content>
        </container>
      </section>
      <section className="Home-7">
        <container className="Home-7-1">
          <content className="Home-7-1">
            <div className="sub-Head-7">
              <h2>We’re calling on amazing talent!</h2>
              <p>
                Join us to revolutionize Africa&apos;s tech landscape with Cloud
                Technology, AI, and Open Source skills.
              </p>
            </div>
            <div className="action">
              <button href="" alt="" className="learn-heading">
                <i>
                  <TbSunset2 />
                </i>
                Learn More
              </button>
              <button className="apply-heading">
                Apply Now
                <i>
                  <IoMdArrowForward />
                </i>
              </button>
            </div>
          </content>
        </container>
      </section>
    </div>
  );
}

export default home;
